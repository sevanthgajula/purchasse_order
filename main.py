...
SHIPPING_DETAILS = """<b>Shipping Address:</b><br/>
ELIXIR PHARMACEUTICALS<br/>
Flat No-304, Sri Sai Apartments<br/>
Opp. Anuradha Timbers<br/>
Chinnathokatta, New Bowenapally<br/>
...
async def generate_po(
    ...
    # Row 1: Company | Shipping | PO Info
    po_info = f"<b>PO Number:</b> {po_number}<br/><b>Date:</b> {date.today().strftime('%d-%m-%Y')}"
    table1 = Table([
...
<b>Payment Terms:</b> {payment_terms}"""
    ...
    product_data = [
        ...
        ['1', Paragraph(f"<b>{product_name}</b><br/>(CAS No-{cas_number})", styles['Normal']),
         hsn_code, unit, str(quantity), inr_format(rate), inr_format(basic_amount)]
    ]
    ...
    # Row 4: Totals Table (Formatted for INR)
    totals_data = [
        ["", "", "BASIC AMOUNT", inr_format(basic_amount)],
        ["", "", "SGST", inr_format(sgst)],
        ["", "", "CGST", inr_format(cgst)],
        ["", "", "IGST", "-" if igst == 0 else inr_format(igst)],
        ["", "", "TAX AMOUNT", inr_format(tax_total)],
        ["", "", "GRAND TOTAL AMOUNT", inr_format(grand_total)]
    ]
    ...
    terms = [
        ...
        "Pl.Ensure the all duties & taxes under GST to be paid intime & Upload the invoices timely and with correct GSTIN, otherwise we shall deduct the GST amount from the bill",
